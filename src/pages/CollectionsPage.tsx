...
function CollectionsCarouselPage() {
  ...
  const scroll = (direction: 'left' | 'right') => {
    ...
    const cardWidth = scrollRef.current.offsetWidth / (isMobile ? 2 : 4)
  ...
  const cardWidth = isMobile ? 'calc(50% - 8px)' : 'calc(25% - 12px)'

  return (
    <div className="min-h-screen" style={{ backgroundColor: '#5c1a1a' }}>
      <div className="px-6 md:px-12 pt-32 md:pt-38 pb-16">
        ...
        >
          {collections.map((col) => (
            <Link
              ...
              style={{ width: cardWidth, scrollSnapAlign: 'start', aspectRatio: '3/4' }}
            >
              <img
                src={col.image}
                alt={col.name}
              ...
              <div className="absolute inset-0 flex items-center justify-center">
                <h2 className="text-[#f5f0eb] text-xl md:text-3xl font-light tracking-widest">{col.name}</h2>
              </div>
            </Link>
          ))}
        </div>
