...
function ProductDetailPage() {
  ...
  if (loading) return <div className="flex items-center justify-center min-h-screen text-sm" style={{ color: '#2c181060', backgroundColor: '#f5f0eb' }}>Loading...</div>
  ...
  function DropdownRow({ label, options, selected, setSelected, name }: {
    ...
    return (
      <div style={{ borderBottom: '1px solid #2c181040' }}>
        <button
          className="w-full flex items-center justify-between py-2"
          onClick={() => setOpenDropdown(isOpen ? null : name)}
        >
          <span className="text-sm" style={{ color: '#2c1810' }}>{label}</span>
          <div className="flex items-center gap-2">
            <span className="text-sm" style={{ color: selected === 'Select' ? '#2c181060' : '#2c1810' }}>
              {selected}
            </span>
            <span style={{ color: '#2c181060', fontSize: '10px' }}>{isOpen ? '∧' : '∨'}</span>
          </div>
        </button>
        {isOpen && (
          <div className="pb-2">
  ...
  return (
    <div className="flex min-h-screen" style={{ backgroundColor: '#f5f0eb' }}>
      ...
      <div className="w-1/2 px-12 pt-32 pb-16 flex flex-col">
        ...
        {/* Tagline + line */}
        <p className="text-sm mb-4" style={{ color: '#2c181070' }}>
          {product.description}
        </p>
        <div style={{ height: '1px', backgroundColor: '#2c181040', marginBottom: '48px' }} />

        {/* Product name */}
        <h1 className="text-4xl font-light mb-16" style={{ color: '#2c1810' }}>
          {product.name}
        </h1>

        {/* Variant selectors */}
        <div style={{ borderTop: '1px solid #2c181040' }}>
        ...
        {/* Add to cart bar */}
        <div className="flex items-center mt-6" style={{ border: '1px solid #2c181050' }}>
          <div className="flex items-center gap-3 px-4 py-2" style={{ borderRight: '1px solid #2c181050' }}>
            <span className="text-sm" style={{ color: '#2c181080' }}>Quantity</span>
            <button onClick={() => setQuantity(q => Math.max(1, q - 1))} className="transition-opacity hover:opacity-50" style={{ color: '#2c1810' }}>−</button>
            <span className="text-sm" style={{ color: '#2c1810' }}>{quantity}</span>
            <button onClick={() => setQuantity(q => q + 1)} className="transition-opacity hover:opacity-50" style={{ color: '#2c1810' }}>+</button>
          </div>
          <div className="flex items-center justify-between flex-1 px-4 py-2">
            ...
            <button
              ...
              style={{ color: added ? '#2c181060' : '#2c1810' }}
            >
